...
from typing import Any, List, Callable, Awaitable, Set, Dict
...
class VoiceClient:
    ...
    async def _send_binary(self, op: _VoiceOpCode, data: bytes) -> None:
        await self._ws.send(op.value.to_bytes(length=1) + data)

    async def _send_heartbeat(self, nonce: int) -> None:
        await self._send(_VoiceOpCode.HEARTBEAT, {"seq_ack": self._last_seq, "t": nonce})
    ...
    async def _identify(self) -> None:
        ...
        await self._send(_VoiceOpCode.IDENTIFY, data)

    async def _send_key_package(self) -> None:
        key_package = self._dave_session_manager.get_key_package_message()
        await self._send_binary(_VoiceOpCode.DAVE_MLS_KEY_PACKAGE, key_package)
        logger.log("OUT", "DAVE MLS KEY PACKAGE")

    async def _handle_hello(self, event: _VoiceEvent) -> None:
    ...
    async def _handle_ready(self, event: _VoiceEvent) -> None:
        ...
        await self._send(_VoiceOpCode.SELECT_PROTOCOL,
                         {"protocol": "udp",
                          "data": {"address": my_ip,
                                   "port": my_port,
    ...
    async def _handle_session_description(self, event: _VoiceEvent) -> None:
        ...
        await self._send(_VoiceOpCode.SPEAKING, speaking_payload)
    ...
    def _handle_dave_mls_external_sender(self, event: _VoiceEvent) -> None:
    ...
    async def _handle_dave_mls_welcome(self, event: _VoiceEvent) -> None:
        ...
        if transition_id == 0:
            ...
            await self._send(_VoiceOpCode.DAVE_TRANSITION_READY, {"transition_id": transition_id})
            logger.log("OUT", f"DAVE TRANSITION READY (transition_id = {transition_id})")

    def _handle_dave_execute_transition(self, event: _VoiceEvent) -> None:
        transition_id = event["transition_id"]
    ...
    async def _handle_dave_mls_proposals(self, event: _VoiceEvent) -> None:
        ...
        match operation_type:
            case 0:  # Append
                ...
                if commit_welcome_message is not None:
                    await self._send_binary(_VoiceOpCode.DAVE_MLS_COMMIT_WELCOME, commit_welcome_message)
    ...
    async def _invalid_commit_recovery(self, transition_id: int) -> None:
        ...
        await self._send(_VoiceOpCode.DAVE_MLS_INVALID_COMMIT_WELCOME, {"transition_id": transition_id})
        logger.log("OUT", f"DAVE MLS INVALID COMMIT WELCOME (transition_id = {transition_id})")

        await self._send_key_package()

    async def _handle_dave_mls_announce_commit_transition(self, event: _VoiceEvent) -> None:
        transition_id = event["transition_id"]
    ...
    def _handle_dave_prepare_transition(self, event: _VoiceEvent) -> None:
        transition_id = event["transition_id"]
    ...
    async def _handle_dave_prepare_epoch(self, event: _VoiceEvent) -> None:
    ...
    async def _reconnect(self) -> None:
        ...
        try:
            ...
            await self._send(_VoiceOpCode.RESUME, resume)
    ...
    async def _handle_disconnection(self, exception: ConnectionClosed) -> bool:
        if self._kicked_or_call_terminated(exception):
    ...
    async def _receive_loop(self) -> None:
        while True:
            try:
                event = _VoiceEvent(await self._ws.recv())
